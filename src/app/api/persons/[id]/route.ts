...
export async function GET(_request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const item = await prisma.person.findUnique({ where: { id }, include: { organizations: { include: { organization: true } } } });
...
export async function PUT(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const validation = validateBody(body, UpdatePersonSchema);
    if (!validation.success) return validation.error;
    const item = await prisma.person.update({ where: { id }, data: validation.data });
