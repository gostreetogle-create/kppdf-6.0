...
import { UpdateOrderClosingSchema } from '@/lib/validations/order-closing';
...
export async function GET(_request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const item = await prisma.orderClosing.findUnique({ where: { id } });
...
export async function PUT(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const validation = validateBody(body, UpdateOrderClosingSchema);
    ...
    const item = await prisma.orderClosing.update({ where: { id }, data: validation.data });
    return apiOk(item);
  } catch (error) {
    if (error instanceof Error && error.message === 'UNAUTHORIZED') return apiError('Не авторизован', 401);
    if (error instanceof Error && error.message === 'FORBIDDEN') return apiError('Доступ запрещён', 403);
...
export async function DELETE(_request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    await requireEditor();
    const { id } = await params;
    await prisma.orderClosing.delete({ where: { id } });
