...
export async function GET(request: NextRequest) {
  try {
    ...
    if (search) {
      where.OR = ['name', 'entity', 'fromStatus', 'toStatus'].map((f) => ({ [f]: { contains: search } }));
    }
    ...
    const [items, total] = await Promise.all([
      prisma.statusWorkflow.findMany({ where, orderBy, skip: (page - 1) * limit, take: limit }),
      prisma.statusWorkflow.count({ where }),
    ]);
