...
import { UpdateWorkCenterSchema } from '@/lib/validations/work-center';
...
export async function GET(_request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const item = await prisma.workCenter.findUnique({ where: { id } });
...
export async function PUT(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const validation = validateBody(body, UpdateWorkCenterSchema);
    if (!validation.success) return validation.error;
    const item = await prisma.workCenter.update({ where: { id }, data: validation.data });
