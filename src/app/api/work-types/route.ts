...
import { CreateWorkTypeSchema } from '@/lib/validations/work-type';
import { validateBody } from '@/lib/validations';
import { getCached, invalidateByPrefix } from '@/lib/cache';

const CACHE_PREFIX = 'work-types';
...
export async function GET(request: NextRequest) {
  try {
    ...
    const result = await getCached(cacheKey, async () => {
      const [items, total] = await Promise.all([
        prisma.workType.findMany({ orderBy, skip: (page - 1) * limit, take: limit }),
...
export async function POST(request: NextRequest) {
  try {
    await requireRole(['admin', 'manager', 'production']); // P2.2: work-type — производственный справочник; production должен иметь доступ к own справочникам
    const body = await request.json();
    const validation = validateBody(body, CreateWorkTypeSchema);
    if (!validation.success) return validation.error;
    const item = await prisma.workType.create({ data: validation.data });
