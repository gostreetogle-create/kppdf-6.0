...
import { UpdateWorkTypeSchema } from '@/lib/validations/work-type';
...
export async function PUT(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const validation = validateBody(body, UpdateWorkTypeSchema);
    if (!validation.success) return validation.error;
    const item = await prisma.workType.update({ where: { id }, data: validation.data });
