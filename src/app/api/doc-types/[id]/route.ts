...
import { UpdateDocTypeSchema } from '@/lib/validations/doc-type';
...
export async function GET(_request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const item = await prisma.docType.findUnique({ where: { id } });
...
export async function PUT(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const validation = validateBody(body, UpdateDocTypeSchema);
    if (!validation.success) return validation.error;
    const item = await prisma.docType.update({ where: { id }, data: validation.data });
