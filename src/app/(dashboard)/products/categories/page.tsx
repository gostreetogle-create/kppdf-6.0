...
export default function ProductCategoriesPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const url = editItem?.id ? `/api/products/categories/${editItem.id}` : '/api/products/categories';
      await fetch(url, {
        method,
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-[var(--foreground)]">Категории товаров</h1>
        <button
          onClick={() => { setEditItem(null); setForm({ name: '', description: '' }); setShowForm(true); }}
      ...
      <div className="bg-[var(--card)] border border-[var(--border)] rounded-xl overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-[var(--border)]">
            {loading ? (
              <tr><td colSpan={3} className="px-4 py-12 text-center text-[var(--muted-foreground)]">Загрузка...</td></tr>
            ) : categories.length === 0 ? (
              <tr><td colSpan={3} className="px-4 py-12 text-center text-[var(--muted-foreground)]">Нет категорий</td></tr>
            ) : categories.map((cat) => (
              <tr key={cat.id} className="hover:bg-[var(--muted)]/50 transition-colors">
                <td className="px-4 py-3 font-medium text-[var(--foreground)]">{cat.name}</td>
                <td className="px-4 py-3 text-[var(--muted-foreground)]">{cat.description || '—'}</td>
                <td className="px-4 py-3 text-right">
                  <div className="flex items-center justify-end gap-1">
                    <button onClick={() => openEdit(cat)} className="p-1.5 rounded hover:bg-[var(--muted)] transition-colors" title="Редактировать">
                      <Pencil className="h-4 w-4 text-[var(--muted-foreground)]" />
                    </button>
                    <button onClick={() => setDeleteConfirm(cat.id)} className="p-1.5 rounded hover:bg-[var(--destructive)]/10 transition-colors" title="Удалить">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center glass-overlay">
          <div className="bg-[var(--card)] border border-[var(--border)] rounded-xl shadow-xl p-6 w-full max-w-sm mx-4">
            ...
            <div className="flex justify-end gap-2">
              <button onClick={() => setDeleteConfirm(null)} className="px-4 py-2 rounded-lg border border-[var(--border)] text-sm hover:bg-[var(--muted)] transition-colors">
                Отмена
              </button>
              <button onClick={() => handleDelete(deleteConfirm)} className="px-4 py-2 rounded-lg bg-[var(--destructive)] text-white text-sm hover:opacity-90 transition-opacity">
                Удалить
              </button>
            </div>
          </div>
