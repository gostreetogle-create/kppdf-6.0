...
- Run 4/5 Склад → нужен `INV-ПРД-CHAIN-КЛАД-*` правила для авто-IN StockMovement при ЗК completed.
- Run 5/5 Финансы → нужен `INV-ПРД-CHAIN-ФИНАНСЫ-*` правила для OrderClosing (completed) + Refund (cancelled) per СПОР-12.
...
| 3 | `in_progress` AND computed `InventoryShortage == true` (`SUM InventoryRecord < SUM ProductionTask.quantityPlanned`) | **AWAITING_MATERIALS** | auto (app-level runtime check, tRPC middleware) |
...
- **AWAITING_MATERIALS:** НЕ derived from `ProductionOrder.status` field. App-level computation через tRPC middleware `requireInventoryCheck()` → SQL JOIN `InventoryRecord < ProductionTask.quantityPlanned`. Schema НЕ требует дополнительных полей на ProductionOrder.
...
4. [ ] Audit log фиксирует derived status с schema-level justification (e.g., "PARTIALLY_READY: ProductionOrder.status='in_progress' + inventoryShortage=false + sumQuantityActual/SumQuantityPlanned=0.65 ≥ 0.5").
...
**Hard-link convention enforcing:** Колонка «Правило» в строках всех CHAIN-*-* групп содержит ТОЛЬКО one-liner `↳ см. INV-XXX-NNN`. Дополнительные ПРД-specific side-effects записываются в MISC-группу или отдельные баннеры НЕ для CHAIN-*.
...
| 20 | `04_Склад/МОДУЛЬ-СКЛАД-ПОДРОБНЫЙ.md` §3 (StockMovement) + §6 (Shipment) | Контекст для auto-IN триггера при ЗК completed → cross-link INV-ПРД-CHAIN-КЛАД-*. |
| 21 | `05_Финансы/МОДУЛЬ-ФИНАНСЫ.md` §5 + §6 (Refund по СПОР-12) | Контекст для auto-OrderClosing (completed) и auto-Refund (cancelled) триггеров → cross-link INV-ПРД-CHAIN-ФИНАНСЫ-*. |
...
|---|---|---|
| `INV-СКЛ-AT-IN-001` (ЗК COMPLETED → StockMovement IN) | `INV-ПРД-CHAIN-КЛАД-001` | При переходе ЗК в `COMPLETED` авто создаётся `StockMovement: type='IN', reason='PRODUCTION'` для каждой ЗК-Задачи с Product.kind='ITEM'. Услуги (`Product.kind='SERVICE'/'WORK'/'INSTALLATION'`) в склад НЕ приходят. |
| `INV-СКЛ-AT-IN-002` (кладовщик видит ЗК как ожидаемую поставку) | `INV-ПРД-CHAIN-КЛАД-002` | В статусе ЗК `IN_PROGRESS` видна «плановая дата прихода» через `StockMovement.expectedAt`. |
| `INV-СКЛ-AT-IN-003` (ЗК Created → placeholder для Склад) | `INV-ПРД-CHAIN-КЛАД-003` | Кладовщик резервирует материалы заранее (manual планирование). |
...
|---|---|---|
| `INV-ФИН-AT-CLOSE-001` (ЗК COMPLETED → OrderClosing) | `INV-ПРД-CHAIN-ФИНАНСЫ-001` | При переходе ЗК в `COMPLETED` авто создаётся `OrderClosing` в Модуле Финансы (по `archive/ANALYSIS-KP-priority.md`, ON DELETE SetNull). |
| `INV-ФИН-REFUND-001` (ЗК CANCELLED → Refund per СПОР-12) | `INV-ПРД-CHAIN-ФИНАНСЫ-002` | При переходе ЗК в `CANCELLED` сигнал в Финансы → бухгалтер оформляет `Refund` (отдельная сущность > 0, вычитается из `Order.paidAmount`). КП остаётся «Оплачено» (per СПОР-12). |
| (нет) | `INV-ПРД-CHAIN-ФИНАНСЫ-003` | Если ЗК отменён ДО оплаты (статус ЗК = CREATED без оплаты) → Refund не оформляется, КП возвращается вручную через «Создать новую версию» (per МОДУЛЬ §10 Q4). |
...
| RBAC-ПРД-A-008 | Видеть «план прихода на Склад» | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | — | INV-ПРД-CHAIN-КЛАД-002 |
| ... (цель ≥20 A-rules) | | | | | | | | | | | |
...
| RBAC-ПРД-OW-003 | storekeeper видит только ЗК-Задачи с Product.kind='ITEM' (to be IN-ventoried) | скрыть в списке | МОДУЛЬ §0 + INV-ПРД-CHAIN-КЛАД-001 |
...
**Hard-link convention:** 4 cross-module CHAIN-*-* groups (Группы 6-9) все правила имеют Source = `INV-XXX-NNN` (NO duplicate of upstream/downstream source rule).
...
|---|---|---|---|
| INV-ПРД-CHAIN-КЛАД-001 | При ЗК COMPLETED авто `StockMovement:IN` для каждой ProductionTask с Product.kind='ITEM' | (Склад Run 4/5 будет `INV-СКЛ-AT-IN-001`) | МОДУЛЬ §6.3 + §5 правило Т-I |
| INV-ПРД-CHAIN-КЛАД-002 | Кладовщик видит ЗК в статусе `IN_PROGRESS` как «ожидаемая поставка» через `StockMovement.expectedAt` | (Склад Run 4/5 будет `INV-СКЛ-AT-IN-002`) | МОДУЛЬ §6.3 |
| INV-ПРД-CHAIN-КЛАД-003 | Услуги (SERVICE/WORK/INSTALLATION) НЕ приходят в склад (skip StockMovement) | (Склад Run 4/5 будет `INV-СКЛ-AT-IN-003` mirror) | МОДУЛЬ §6.3 + §5.2 правило 2 |
...
|---|---|---|---|
| INV-ПРД-CHAIN-ФИНАНСЫ-001 | При ЗК COMPLETED → авто OrderClosing в Модуль Финансы (ЗК-финансовая фиксация) | (Финансы Run 5/5 будет `INV-ФИН-AT-CLOSE-001`) | МОДУЛЬ §6.4 + СПОР-12 |
| INV-ПРД-CHAIN-ФИНАНСЫ-002 | При ЗК CANCELLED → сигнал в Финансы для оформления Refund (КП остаётся «Оплачено») | (Финансы Run 5/5 будет `INV-ФИН-REFUND-001`) | МОДУЛЬ §10 Q1 + СПОР-12 |
| INV-ПРД-CHAIN-ФИНАНСЫ-003 | Если ЗК отменён ДО оплаты (ЗК CREATED без оплаты) → Refund не оформляется, КП возвращается вручную через «Создать новую версию» | (Финансы Run 5/5 будет `INV-ФИН-REFUND-002` mirror) | МОДУЛЬ §10 Q4 |
...
| INV-ПРД-TERM-003 | ЗК CANCELLED = final, никаких восстановлений | UI-flow: НЕТ кнопок из CANCELLED | МОДУЛЬ §11 + INV-ПРД-CHAIN-ФИНАНСЫ-002 |
...
| SM-ПРД-T-008 | IN_PROGRESS / PARTIALLY_READY | COMPLETED | авто (когда ВСЕ задачи task_DONE) | (система) | все tasks task_DONE | **🔴 АВТО КЛАД**: StockMovement IN для каждой ITEM; **🔴 АВТО ФИНАНСЫ**: OrderClosing в Финансах | INV-ПРД-CHAIN-КЛАД-001 + INV-ПРД-CHAIN-ФИНАНСЫ-001 |
...
| SM-ПРД-T-011 | любой (кроме ARCHIVED, CANCELLED) | CANCELLED | «Расторгнуть» | admin / director / production-руководитель | письменное уведомление клиента + file-доказательство | **🔴 АВТО ФИНАНСЫ**: Сигнал на Refund (по СПОР-12); КП остаётся «Оплачено» | INV-ПРД-TERM-* + INV-ПРД-CHAIN-ФИНАНСЫ-002 |
