...
4. [ ] Audit log фиксирует derived status с schema-level justification (e.g., "PARTIALLY_READY: `ProductionOrder.status='in_progress'` AND `sumQuantityActual/SumQuantityPlanned=0.65 ≥ 0.5` (ratio computed server-side)" — criteria A — НЕ должно ссылаться на inventory shortage (это для AWAITING_MATERIALS)).
